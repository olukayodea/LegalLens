<?php
	...
	if (isset($_REQUEST['s'])) {
		...
		$list = $articles->fullSearch($s, $id, $filter);
		$tag = "Search Result for <strong>'".$s."'</strong>".$tag2."";
	} else if (isset($_REQUEST['q'])) {
		$q = strtoupper($common->get_prep($_REQUEST['q']));
		$list = $articles->indexSearch($q, $id, $filter);
	} else {
		$list = $articles->listAllHome($id, $filter);
	}
?>
<!doctype html>
...
<div class="span7">
   <div style="border:1px solid #ccc; padding:10px">
     <div style="margin-top:30px">
       <h4 style="" align="center">Articles and Journals</h4>
        ...
        <div id="easyPaginate">
			<?php foreach ($list as $key => $value) { ?>
                ...
                <p><strong><a href="<?php echo URL; ?>articles.view?id=<?php echo $list[$key][$i]['ref']; ?>"><?php echo $list[$key][$i]['title']; ?></a></strong><br>Document type <?php echo $list[$key][$i]['type']; ?></p>
                <?php } ?>
            <?php } ?>
        </div>
	 </div>
   ...
   <!-- <script type='text/javascript' src='js/jquery-1.8.3.min.js'></script> -->
	...
	<script>
        $(function() {
            ...
            $( "#s" ).catcomplete({
      		  delay: 0,
              source: "includes/scripts/auto_complete_art.php?type=<?php echo $autoSearch; ?>",
				select: function( event, ui ) {
					window.location='articles.view?id='+ui.item.code;
				}
            });
        });
