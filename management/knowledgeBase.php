<?php
	...
	if (isset($_GET['editAdmin'])) {
		$getData = $knowledge_base->getOne($common->get_prep($_GET['id']));
		$agearray = explode(",", $getData['category']);
		
		$editID = $getData['id'];
		$editAdmin = true;
			
	} else if (isset($_GET['deleteAdmin'])) {
		$edit = $knowledge_base->remove($common->get_prep($_GET['id']));
		if ($edit) {
			header("location: ?done");
		} else {
			header("location: ?error");
	...
	if (count($listCat) < 1) {
		header("location: knowledgeBase.categories?error=".urlencode("You must create a category first"));
	}

	$listAdmin = $knowledge_base->listAll();
?>
<!DOCTYPE html>
<html>
<head>
    ...
    <div class="wrapper">
      ...
      <div class="content-wrapper">
        ...
        <section class="content-header">
          <h1>
            Help
            <small>Manage KnowledgeBase</small>
          </h1>
          <ol class="breadcrumb">
          ...
          <div class="row"><div class="col-md-12">
            ...
            <div class="box box-primary">
                <div class="box-header with-border">
                  <h3 class="box-title">Add New Knowledge Base</h3>
                </div><!-- /.box-header -->
                <!-- form start -->
                <form role="form" action="" method="post">
                  ...
                  <div class="form-group">
                      <label for="validity">Category <em>*</em><br />
                    <p class="help-block">select the category for this knowledge base</p></label><br />
                    <span id="spryselect1">
                      <select multiple name="category[]" class="form-control select2" id="category"<?php if (isset($_REQUEST['view'])) { ?> disabled<?php } ?>>
                      <?php for ($i = 0; $i < count($listCat); $i++) {?>
                      <option value="<?php echo $listCat[$i]['ref']; ?>"<?php if (in_array($listCat[$i]['ref'], $agearray)) { ?> selected<?php } ?>><?php echo $listCat[$i]['title']; ?></option>
                      <?php } ?>
                      </select>
