<?php
	...
	if ((isset($_POST['addAdmin'])) || (isset($_POST['editButton']))) {
		$add = $articles->add($_POST);
		if ($add) {
			if (isset($_POST['editButton'])) {
				header("location: ?done");
			} else {
				header("location: article.sections?id=$add");
			}
		} else {
	...
	if (isset($_GET['editAdmin'])) {
		$getData = $articles->getOne($common->get_prep($_GET['id']));
		$editAdmin = true;
			
	} else if (isset($_GET['deleteAdmin'])) {
		$edit = $articles->remove($common->get_prep($_GET['id']));
		if ($edit) {
			header("location: ?done");
		} else {
			header("location: ?error");
    ...
    <div class="wrapper">
      ...
      <div class="content-wrapper">
        ...
        <section class="content-header">
          <h1> Articles and Journals <small>Manage Articles and Journals</small> </h1>
          <ol class="breadcrumb">
          ...
          <div class="row"><div class="col-md-12">
            ...
            <div class="box box-primary">
                <div class="box-header with-border">
                  <h3 class="box-title">Add New Articles or journal</h3>
                </div><!-- /.box-header -->
                <!-- form start -->
    ...
    <?php } ?>
                  ...
                  <div class="form-group">
                    ...
                    <select name="type" id="type" class="form-control select2" style="width: 100%;" required>
                      ...
                      <option<?php if ($getData['type'] == "Article") { ?> selected<?php } ?> value="Article">Article</option>
                      <option<?php if ($getData['type'] == "Journals") { ?> selected<?php } ?> value="Journals">Journals</option>
                    </select>
                    <span class="selectRequiredMsg">Please select an item.</span></span></div>
<?php if ($editAdmin === true) { ?>
                    <div class="form-group">
                    <label> <a href="<?php echo URLAdmin; ?>article.sections?id=<?php echo $getData['ref']; ?>">Click here to view/modify sections in this article/journal</a></label>
                    </div>
                    <?php } ?>
                  </div><!-- /.box-body -->
            ...
            <div class="col-xs-12">
              <div class="box">
                <div class="box-header">
                  <h3 class="box-title">Showing All Articles and Journals</h3>
                </div><!-- /.box-header -->
                <div class="box-body">
