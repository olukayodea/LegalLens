<?php
	class search extends common {
		function create($array, $page_count) {
			...
			if (is_array($array['parameter'])) {
				for ($i = 0; $i < count($array['parameter']); $i++) {
					...
					$cat = ltrim($cat, ",");
					$result['doc'][$array['parameter'][$i]] = $this->searchCategory($val, $cat, $page_count);
				}
			}
			if ($array['case_law'] == 1) {
				$result['case_law'] = $this->searchCase($val, $page_count);
			}
			if ($array['reg_circular'] == 1) {
				$result['reg'] = $this->searchregulation($val, $page_count);
			}
			if ($array['dic'] == 1) {
				$result['dic'] = $this->searchDictionar($val, $page_count);
			}
			return $result;
		}
