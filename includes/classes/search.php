<?php
	class search extends common {
		...
		function searchCase($val) {
			...
			$sql = mysql_query("SELECT `caselaw`.`title`, `reporter`, `caselaw`.`court`, `caselaw`.`file`,  `caselaw`.`year`, `caselaw`.`create_time`, `caselaw`.`modify_time`, `caselaw`.`ref`, `caselaw_sections`.`section_content`, `caselaw_sections`.`tags`, `caselaw_sections`.`ref` AS 'section_ID', MATCH (`caselaw_sections`.`section_content`) AGAINST ('".$val."' IN BOOLEAN MODE) AS name_match, citation FROM `caselaw`, `caselaw_sections` WHERE `caselaw`.`ref` = `caselaw_sections`.`caselaw` AND `caselaw`.`status` = 'active' AND `caselaw_sections`.`status` = 'active' AND (`caselaw`.`court` LIKE '%".$val."%' OR `caselaw`.`year` LIKE '%".$val."%' OR `caselaw`.`title` LIKE '%".$val."%' OR `caselaw_sections`.`citation` LIKE '%".$val."%' OR `caselaw_sections`.`tags` LIKE '%".$val."%' OR `caselaw_sections`.`section_content` LIKE '%".$val."%' OR MATCH(`caselaw_sections`.`section_content`) AGAINST ('".$val."') OR (MATCH(`caselaw_sections`.`section_content`) AGAINST ('".$val."'))) GROUP BY `caselaw_sections`.`section_content` ORDER BY  `name_match` DESC,`caselaw_sections`.`tags`, `title`,`court` ASC") or die (mysql_error());
		...
		function searchregulation($val) {
			$sql = mysql_query("SELECT `regulations`.`title`, `regulations`.`regulator`, `regulations`.`create_time`, `regulations`.`modify_time`, `regulations`.`ref`,`regulations_sections`.`ref` AS `section_ref`, `regulations_sections`.`section_content`, `regulations_sections`.`section_no`, MATCH (`regulations`.`tags`) AGAINST ('".$val."' IN BOOLEAN MODE) AS name_match, MATCH (`regulations_sections`.`section_content`) AGAINST ('".$val."' IN BOOLEAN MODE) AS name_match2, MATCH (`regulations_sections`.`tags`) AGAINST ('".$val."' IN BOOLEAN MODE) AS name_match3  FROM `regulations`, `regulations_sections` WHERE `regulations`.`ref` = `regulations_sections`.`regulations` AND `regulations`.`status` = 'active' AND `regulations_sections`.`status` = 'active' AND (`regulations`.`year` LIKE '%".$val."%' OR `regulations`.`title` LIKE '%".$val."%' OR `regulations`.`tags` LIKE '%".$val."%' OR `regulations_sections`.`tags` LIKE '%".$val."%' OR `regulations_sections`.`section_no` LIKE '%".$val."%' OR `regulations_sections`.`section_content` LIKE '%".$val."%' OR (MATCH(`regulations_sections`.`tags`) AGAINST ('".$val."') OR MATCH(`regulations`.`tags`) AGAINST ('".$val."') OR MATCH(`regulations`.`title`) AGAINST ('".$val."') OR MATCH(`regulations_sections`.`section_content`) AGAINST ('".$val."'))) GROUP BY `regulations_sections`.`section_content` ORDER BY `name_match3`,`name_match2`,`name_match` DESC") or die (mysql_error());
		...
		function searchDictionar($val) {
			$sql = mysql_query("SELECT `ref`, `title`, `details`, MATCH (`details`) AGAINST ('".$val."' IN BOOLEAN MODE) AS name_match  FROM `list_library` WHERE `list_library`.`status` = 'active' AND (`list_library`.`title` LIKE '%".$val."%' OR `list_library`.`details` LIKE '%".$val."%' OR MATCH(`details`) AGAINST ('".$val."')) GROUP BY `details` ORDER BY `name_match` DESC, `title` ASC") or die (mysql_error());
		...
		function searchForumCat($val) {
			$sql = mysql_query("SELECT `cat_id`, `cat_name`, `cat_description`, MATCH(`cat_description`) AGAINST ('".$val."' IN BOOLEAN MODE) AS name_match FROM `forum_categories` WHERE  `forum_categories`.`status` = 'active' AND (`cat_name` LIKE '%".$val."%' OR `cat_description` LIKE '%".$val."%' OR MATCH(`cat_description`) AGAINST ('".$val."')) ORDER BY `cat_name` ASC") or die (mysql_error());
		...
		function searchForumTitle($val) {
			$sql = mysql_query("SELECT `topic_id`, `topic_subject`, MATCH(`topic_subject`) AGAINST ('".$val."' IN BOOLEAN MODE) AS name_match FROM `forum_topics` WHERE  `forum_topics`.`status` = 'active' AND (`topic_subject` LIKE '%".$val."%' OR MATCH(`topic_subject`) AGAINST ('".$val."')) ORDER BY `topic_id` ASC") or die (mysql_error());
		...
		function searchForumPost($val) {
			$sql = mysql_query("SELECT `post_id`, `post_content`, MATCH(`post_content`) AGAINST ('".$val."' IN BOOLEAN MODE) AS name_match FROM `forum_posts` WHERE  `forum_posts`.`status` = 'active' AND (`post_content` LIKE '%".$val."%' OR MATCH(`post_content`) AGAINST ('".$val."')) ORDER BY `post_id` ASC") or die (mysql_error());
