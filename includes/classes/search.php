<?php
	class search extends common {
		...
		function searchCategory($val, $cat, $page_count) {
			...
			try {
				$sql = $db->prepare("SELECT `documents`.`title`, `documents`.`ref`, `sections`.`ref` AS section_ref, `documents`.`cat`, `documents`.`type`, `documents`.`year`, `documents`.`category_name`, `documents`.`owner`, `sections`.`section_no`, `sections`.`section_content`, `documents`.`create_time`, `documents`.`modify_time`,`categories`.`priority`,MATCH (`sections`.`section_content`) AGAINST ('?' IN BOOLEAN MODE) AS name_match FROM `documents`, `sections`,`categories` WHERE `documents`.`cat` = `categories`.`ref` AND`documents`.`ref` = `sections`.`document` AND `documents`.`status` = 'active' AND `sections`.`status` = 'active' AND `cat` IN (".$cat.") AND (`documents`.`title` LIKE '%?%' OR `documents`.`year` LIKE '%?%' OR `documents`.`category_name` LIKE '%?%' OR `documents`.`owner` LIKE '%?%' OR `documents`.`tags` LIKE '%?%' OR `sections`.`tags` LIKE '%?%' OR `sections`.`section_content` LIKE '%?%' OR MATCH(`sections`.`section_content`) AGAINST ('?') OR MATCH(`documents`.`title`) AGAINST ('?')) GROUP BY `sections`.`section_content` ORDER BY `name_match` DESC, `documents`.`tags`,`title`,`documents`.`category_name` ASC LIMIT ".$limit.",".page_list);
				$sql->execute([$val]);
		...
		function searchCase($val, $page_count) {
			...
			$limit = intval(page_list*$page_count);
			try {
				$sql = $db->prepare("SELECT `caselaw`.`title`, `reporter`, `caselaw`.`court`, `caselaw`.`file`,  `caselaw`.`year`, `caselaw`.`create_time`, `caselaw`.`modify_time`, `caselaw`.`ref`, `caselaw_sections`.`section_content`, `caselaw_sections`.`tags`, `caselaw_sections`.`ref` AS 'section_ID', MATCH (`caselaw_sections`.`section_content`) AGAINST ('?' IN BOOLEAN MODE) AS name_match, citation FROM `caselaw`, `caselaw_sections` WHERE `caselaw`.`ref` = `caselaw_sections`.`caselaw` AND `caselaw`.`status` = 'active' AND `caselaw_sections`.`status` = 'active' AND (`caselaw`.`court` LIKE '%?%' OR `caselaw`.`year` LIKE '%?%' OR `caselaw`.`title` LIKE '%?%' OR `caselaw_sections`.`citation` LIKE '%?%' OR `caselaw_sections`.`tags` LIKE '%?%' OR `caselaw_sections`.`section_content` LIKE '%?%' OR MATCH(`caselaw_sections`.`section_content`) AGAINST ('?') OR (MATCH(`caselaw_sections`.`section_content`) AGAINST ('?'))) GROUP BY `caselaw_sections`.`section_content` ORDER BY  `name_match` DESC,`caselaw_sections`.`tags`, `title`,`court` ASC LIMIT ".$limit.",".page_list);
				$sql->execute([$val]);
			} catch(PDOException $ex) {
				echo "An Error occured! ".$ex->getMessage(); 
			}
		...
		function searchDictionar($val, $page_count) {
			...
			$limit = intval(page_list*$page_count);
			try {
				$sql = $db->prepare("SELECT `ref`, `title`, `details`, MATCH (`details`) AGAINST ('?' IN BOOLEAN MODE) AS name_match  FROM `list_library` WHERE `list_library`.`status` = 'active' AND (`list_library`.`title` LIKE '%?%' OR `list_library`.`details` LIKE '%?%' OR MATCH(`details`) AGAINST ('?')) GROUP BY `details` ORDER BY `name_match` DESC, `title` ASC LIMIT ".$limit.",".page_list);
				$sql->execute([$val]);
			} catch(PDOException $ex) {
				echo "An Error occured! ".$ex->getMessage(); 
			}
		...
		function searchDictionarCount($val) {
			global $db;
			try {
				$sql = $db->prepare("SELECT `ref`, `title`, `details`, MATCH (`details`) AGAINST ('?' IN BOOLEAN MODE) AS name_match  FROM `list_library` WHERE `list_library`.`status` = 'active' AND (`list_library`.`title` LIKE '%?%' OR `list_library`.`details` LIKE '%?%' OR MATCH(`details`) AGAINST ('?')) GROUP BY `details`");
				$sql->execute([$val]);
			} catch(PDOException $ex) {
				echo "An Error occured! ".$ex->getMessage(); 
			}
