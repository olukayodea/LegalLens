<?php
	$redirect = "mobile_agreements";
	include_once("includes/functions.php");
	//include_once("includes/session.php");
	
	if (isset($_REQUEST['s'])) {
		$s = $common->get_prep($_REQUEST['s']);
		$list = $drafting->fullSearch($s, 'Agreement');
		$tag = "Search Result for <strong>'".$s."'</strong>";
	} else if (isset($_REQUEST['q'])) {
		$q = $common->get_prep($_REQUEST['q']);
		$list = $drafting->indexSearch($q, 'Agreement');
	} else {
		$list = $drafting->listAllHome("Agreement");
	}
?>
<!doctype html>
...
<script>
  (adsbygoogle = window.adsbygoogle || []).push({
    ...
    <title>Legal Drafting >> Agreements </title>
    <?php $pages->head(); ?>
<style>
...
<h4 style="" align="center">Agreements</h4>
...
<div id="easyPaginate">
	<?php foreach ($list as $key => $value) { ?>
        ...
        <p><strong><a href="<?php echo URL; ?>mobile_clause.data?view=Agreement&id=<?php echo $list[$key][$i]['ref']; ?>"><?php echo $list[$key][$i]['title']; ?></a></strong></p>
        <?php } ?>
    <?php } ?>
   ...
   <!-- <script type='text/javascript' src='js/jquery-1.8.3.min.js'></script> -->
	...
	<script>
        $(function() {
            ...
            $( "#s" ).catcomplete({
      		  delay: 0,
              source: "includes/scripts/auto_complete_clause.php?type=Agreement",
				select: function( event, ui ) {
					window.location='mobile_clause.data?view='+ui.item.type+"&id="+ui.item.code;
				}
            });
        });
